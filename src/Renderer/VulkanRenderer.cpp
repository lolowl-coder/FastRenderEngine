...
namespace fre
{
	...
	uint32_t VulkanRenderer::createBLAS(MeshPtr& mesh)
	{
		...
		uint32_t bufferIndex = createBuffer(
			buffer_usage_flags,
	...
	VkAccelerationStructureInstanceKHR VulkanRenderer::createBlasInstance(uint32_t blasId, const mat4& matrix)
	{
		...
		asDeviceAddressInfo.accelerationStructure = blas.mHandle;
	...
	uint32_t VulkanRenderer::buildAccelerationStructure(VkAccelerationStructureGeometryKHR& asGeometry,
		const VkAccelerationStructureTypeKHR asType, const uint32_t primitiveCount)
	{
		...
		auto result = mAccelerationStructures.size();
		...
		asCreateInfo.buffer = as.mBuffer.mBuffer;
		asCreateInfo.size = asBuildSizesInfo.accelerationStructureSize;
		asCreateInfo.type = asType;
		vkCreateAccelerationStructureKHR(mainDevice.logicalDevice, &asCreateInfo, nullptr, &as.mHandle);

		// The actual build process starts here

		// Create a scratch buffer as a temporary storage for the acceleration structure build
		bufferIndex = createBuffer(
		...
		asBuildGeometryInfo.dstAccelerationStructure = as.mHandle;
		asBuildGeometryInfo.geometryCount = 1;
		asBuildGeometryInfo.pGeometries = &asGeometry;
		asBuildGeometryInfo.scratchData.deviceAddress = scratchBuffer.mDeviceAddress;
		...
		commandBuffer.allocate(mGraphicsCommandPool, mainDevice.logicalDevice);
		commandBuffer.begin();
		...
		fenceInfo.flags = 0;
		VkFence fence;
		VK_CHECK(vkCreateFence(mainDevice.logicalDevice, &fenceInfo, nullptr, &fence));
		...
		asDeviceAddressInfo.accelerationStructure = as.mHandle;
		as.mDeviceAddress = vkGetAccelerationStructureDeviceAddressKHR(mainDevice.logicalDevice, &asDeviceAddressInfo);
	...
	AccelerationStructure& VulkanRenderer::getAS(const uint32_t id)
	{
		assert(id < mAccelerationStructures.size());
		
		return mAccelerationStructures[id];
	}
    ...
    {
		const Shader* result = nullptr;

        if(shaderId < mShaders.size())
        {
            result = &mShaders[shaderId];
        }
        else
        {
            LOG_ERROR("Shader with ID {} not found", shaderId);
        }

		return result;
    }

	std::vector<const VulkanShader*> VulkanRenderer::getRTShaders(const uint32_t shaderId)
	{
		...
		for(auto& shader : mShaders)
		{
			...
			for(const auto& shaderMetaData : shaderMetaDatum)
			{
					...
					shader.mRayClosestHitShader.mShaderModule != VK_NULL_HANDLE)
				{
					...
					mPipelines.push_back(VulkanPipeline());
					auto& pipeline = mPipelines.back();
                    std::vector<const VulkanShader*> rtShaders = getRTShaders(shader.mId);
					pipeline.createRTPipeline(
						mainDevice.logicalDevice, rtShaders,
						dsls,
						shaderMetaData.mPushConstantRanges);
