...
// Rendering equation used in this project:
// Lo(x, V) = Le(x, V) + SumOverSamplesCount(Fr(x, L, V) * Li(x, L) * NdotL)
// Below is the breakdown of it.
...
// Assuming rendering equation is:
// Lo(x, V) = Le(x, V) + SumOverSamplesCount(Fr(x, L, V) * Li(x, L) * NdotL)
// Where:
...
// this function calculates following part of rendering equation:
// SumOverSamplesCount(Fr(x, L, V) * Li(x, L) * NdotL)
// Emissive term Le(x, V) should be added separately.
